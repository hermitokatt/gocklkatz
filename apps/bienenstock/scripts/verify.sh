#!/usr/bin/env bash
# Proves Bienenstock RUNS, which scripts/ci.sh cannot.
#
...
# Env:    VERIFY_PORT  override the port (default 43126; 43123 is Ameisenwerkstatt, 43124 the
#                      landing page, 43125 Simplified)
#
...
LEAKED=0
FAILED=0
LOG="$(mktemp "${TMPDIR:-/tmp}/bienenstock-verify.XXXXXX")" || exit 2
...
say "route GET /                       $code"
if [ "$code" = "200" ] && printf '%s' "$html" | grep -q 'Bienenstock'; then
    ok "GET / is 200 and names the app"
else
    bad "GET / did not answer 200 with the app's name (got $code)"
fi

# /bienen — the scene page: attribute + rendered content, not status alone
body="$(curl -sS -m 10 -w '\n%{http_code}' "$BASE/bienen" 2>/dev/null)"
code="$(printf '%s' "$body" | tail -1)"
html="$(printf '%s' "$body" | sed '$d')"
...
if printf '%s' "$body" | grep -q '"ok":true' \
    && printf '%s' "$body" | grep -q '"service":"bienenstock"'; then
