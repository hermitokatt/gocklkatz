/** Minimum CSS height for the colony canvas (matches `.frame` / prior Werkstatt CSS). */
export const CANVAS_MIN_CSS_HEIGHT = 420;
...
/**
 * Resolve canvas CSS + bitmap sizes without a ResizeObserver feedback loop (STE-42).
 *
 * When `height: 100%` does not resolve (CSS missing / offline chunk), browsers use the
 * canvas bitmap intrinsic size as layout height. Writing `canvas.height` from
 * `getBoundingClientRect() × devicePixelRatio` then grows without bound and stretches
 * the left rail (`repeat(5, minmax(0, 1fr))`).
 *
 * If the newly measured box matches the previous bitmap size, keep the prior CSS size.
 */
